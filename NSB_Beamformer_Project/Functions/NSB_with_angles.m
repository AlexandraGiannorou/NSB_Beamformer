function [result] = NSB_with_angles(SNR, theta_original)
    %this is the null steering beamforming algorithm as given while
    %adding fictitious angles in attempt to improve side lobe level
    %perfomance
    
   
    theta=adding_angles(theta_original);
    ...
    for i=1:6
        deviations(i)=min(abs(locations2-theta_original(i)));        
    end
